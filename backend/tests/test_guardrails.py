...
from tests.fakes import ScriptedAnthropic, text_message
...
async def test_classifier_blocks_adversarial_prompt(prompt, category):
    ...
    anthropic = ScriptedAnthropic(conversation_script=[], guardrail_verdict=verdict_json)
...
async def test_classifier_fails_closed_on_unparseable_response():
    anthropic = ScriptedAnthropic(conversation_script=[], guardrail_verdict="not json at all")
    classifier = GuardrailsClassifier(
        create_message=anthropic,
        model="claude-haiku-4-5-20251001",
        categories=DEFAULT_CATEGORIES,
...
async def test_classifier_disabled_never_blocks_and_skips_the_call():
    anthropic = ScriptedAnthropic(conversation_script=[], guardrail_verdict='{"blocked": true, "category": "hate_speech", "reason": "x"}')
    classifier = GuardrailsClassifier(
        create_message=anthropic,
        model="claude-haiku-4-5-20251001",
        categories=DEFAULT_CATEGORIES,
...
async def test_blocked_input_never_reaches_a_tool_call(prompt, category):
    ...
    # the tool-use loop, ScriptedAnthropic would raise "ran out of scripted turns".
    orchestrator, anthropic, mcp = build_orchestrator(
